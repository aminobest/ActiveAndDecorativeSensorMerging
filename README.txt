...
2. A Jar Runnable version of ProM with ADSM plugin can be executed from runProMwithADSM.sh (Test dataset available at https://github.com/aminobest/ActiveAndDecorativeSensorMerging/tree/master/ADSM%20evaluation/100cases/100casesR4 . The files generated by the simulator (to be used for testing) have the prefix “AmaSmartLog_”). Note that the Case Id might not be correctly inferred with small datasets, which is the case for AmaSmartLog_clerkProcess.csv log. It is recommended to check the correctness of the Case Id index before applying the merging. 
